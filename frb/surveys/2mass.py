...
for band in MASS_bands:
    photom["2MASS"]["2MASS"+'_{:s}'.format(band)] = '{:s}_mag'.format(band.lower())
    photom["2MASS"]["2MASS"+'_{:s}_err'.format(band)] = '{:s}_magerr'.format(band.lower())
    photom["2MASS"]["GALEX_ID"] = 'objID'
photom["2MASS"]['ra'] = 'ra'
...
_DEFAULT_query_fields +=['{:s}_mag'.format(band) for band in MASS_bands]
_DEFAULT_query_fields +=['{:s}_magerr'.format(band) for band in MASS_bands]

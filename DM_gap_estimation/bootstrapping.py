...
DM_rand_draws_sample_100 = np.asarray(random.sample(list(DM_rand_draws),100))
...
for i in range(len(DM_grid)):
    var100_ = np.var(new_sampled_values_100[i])
    var100 = np.append(var100,var100_)

var1000 = []
for i in range(len(DM_grid)):
    var1000_ = np.var(new_sampled_values_1000[i])
    var1000 = np.append(var1000,var1000_)

varobs = []
for i in range(len(DM_grid)):
    varobs_ = np.var(new_sampled_values_obs[i])
...
plt.ylabel('Variance')
plt.tight_layout()
plt.savefig('bootstrap_outputs/variances.png')
plt.show()
